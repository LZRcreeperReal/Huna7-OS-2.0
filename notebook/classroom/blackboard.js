...
Huna7.Blackboard = (() => {
  ...
  const _applyWallpaper = () => {
    ...
    const wp = Huna7.Notebook?.Wallpaper?.getCurrent?.(); const bg = wp && wp.type === 'image' ? `url("${wp.value}")` : Huna7.Dictionary.getWallpaperCSS(wp?.value || theme?.wallpaper || 'gradient-aurora');
    _root.style.cssText = `width:100%;height:100%;position:relative;overflow:hidden;background:${bg};`;
    // Subtle noise overlay
    const noise = document.createElement('div');
    noise.style.cssText = `position:absolute;inset:0;pointer-events:none;z-index:0;
      opacity:0.025;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E");`;
