...
Huna7.Startup.Launch = (() => {
  ...
  const _showSetupWizard = () => {
    const ui = _buildScreen('h7-setup-screen');
    let step = 1;
  ...
  // ── Login Screen ──────────────────────────────────────
  // _renderLoginIntoCard renders login UI into an existing card element.
  // This lets the reset flow swap back to login WITHOUT rebuilding the screen,
  // which fixes the top-left corner bug (screen position is preserved).
  const _renderLoginIntoCard = (ui, username) => {
    ...
    Huna7.Startup.Logo.render(logoWrap, { size: 64, animated: false });
    ...
    // Pass the same ui object so reset can swap back into it
    resetLink.addEventListener('click', () => _renderResetIntoCard(ui, username));
  ...
  const _showLoginScreen = () => {
    ...
    _renderLoginIntoCard(ui, username);
  };

  // ── Reset Flow — renders into the same card as login ──
  // This means the card stays centred — no new screen is created.
  const _renderResetIntoCard = (ui, username) => {
    const steps = [
    ...
    const render = () => {
      ...
      text.style.cssText = 'font-size:13px;color:rgba(255,255,255,0.55);line-height:1.7;padding:2px 0 8px;';
      text.textContent = steps[step].text;

      const confirmBtn = _mkBtn(
        step < 2 ? 'I understand, continue' : 'Reset Now',
        'danger',
        async () => {
      ...
      const cancelBtn = _mkBtn('Cancel', 'ghost', () => {
        // Swap back to login UI inside the same card — card stays centred
        _renderLoginIntoCard(ui, username);
      });
