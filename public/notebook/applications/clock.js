  ...
  const launch = (pid, options = {}) => {
    ...
    const renderClock = () => {
      ...
      const drawClock = () => {
        ...
        drawHand((s / 60) * Math.PI * 2 - Math.PI / 2, r * 0.82, 1, 'var(--h7-accent)');

        // Center dot
        ctx.beginPath(); ctx.arc(cx, cy, 4, 0, Math.PI*2);
        ctx.fillStyle = 'var(--h7-accent)'; ctx.fill();
    ...
    const renderStopwatch = () => {
      ...
      const tick = () => { display.textContent = fmt(swRunning ? swElapsed + Date.now() - swStart : swElapsed); };
