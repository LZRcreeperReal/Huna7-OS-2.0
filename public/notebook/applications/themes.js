...
Huna7.Apps.Themes = (() => {
  function launch(pid, options = {}) {
    const { id, contentEl } = Huna7.Desk.createWindow({
      title: 'Theme Studio', appId: 'themes', width: 940, height: 640,
    });
    contentEl.style.display = 'flex';
    contentEl.style.background = 'linear-gradient(145deg, var(--h7-bg-panel), var(--h7-bg))';

    let workingTheme = { ...Huna7.Encyclopedia.getCurrent() };
    const originalTheme = { ...workingTheme };

    if (!document.getElementById('h7-theme-studio-styles')) {
      const style = document.createElement('style');
      style.id = 'h7-theme-studio-styles';
      style.textContent = `
        .h7-theme-card { border:1px solid transparent; border-radius:10px; transition:background 150ms ease,border-color 150ms ease,transform 150ms ease; }
        .h7-theme-card:hover { background:var(--h7-bg-glass-hover); transform:translateX(2px); }
        .h7-theme-card.is-selected { background:var(--h7-bg-glass-hover); border-color:color-mix(in srgb, var(--h7-accent) 55%, transparent); }
        .h7-theme-field { padding:11px; border:1px solid var(--h7-border); border-radius:10px; background:rgba(255,255,255,0.025); }
        .h7-theme-field:focus-within { border-color:var(--h7-accent); box-shadow:0 0 0 3px color-mix(in srgb, var(--h7-accent) 16%, transparent); }
      `;
    ...
    sidebar.style.cssText = 'width:230px;border-right:1px solid var(--h7-border);display:flex;flex-direction:column;background:rgba(0,0,0,0.10);';
    const sidebarTitle = document.createElement('div');
    sidebarTitle.style.cssText = 'padding:18px 16px 14px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:var(--h7-text-muted);border-bottom:1px solid var(--h7-border);';
    sidebarTitle.innerHTML = 'Theme Library<br><span style="font-size:12px;letter-spacing:0;text-transform:none;font-weight:400;display:block;margin-top:6px;color:var(--h7-text);">Choose a starting point</span>';
    ...
    toolbar.style.cssText = 'display:flex;align-items:center;gap:8px;padding:14px 18px;border-bottom:1px solid var(--h7-border);flex-shrink:0;background:rgba(255,255,255,0.025);';
    ...
    revertBtn.className = 'h7-btn h7-btn-ghost';
    revertBtn.textContent = 'Revert';
    revertBtn.title = 'Restore the theme that was active when Theme Studio opened';
    revertBtn.addEventListener('click', () => {
      Object.assign(workingTheme, originalTheme);
      themeName.value = originalTheme.name || '';
      Huna7.Encyclopedia.apply(originalTheme);
      syncInputs();
      Huna7.Bulletin.success('Theme Studio', 'Reverted preview');
    });
    ...
    toolbar.append(themeName, applyBtn, saveBtn, revertBtn, exportBtn, importBtn);

    // Color grid
    const colorGrid = document.createElement('div');
    colorGrid.style.cssText = 'flex:1;overflow-y:auto;padding:18px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;align-content:start;';
    ...
    preview.style.cssText = 'padding:14px 18px;border-top:1px solid var(--h7-border);flex-shrink:0;background:rgba(0,0,0,0.12);';
    preview.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;"><div class="h7-label" style="margin:0;">Live preview</div><span style="font-size:11px;color:var(--h7-text-muted);">Changes appear across the OS immediately</span></div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <button class="h7-btn h7-btn-accent">Primary action</button>
        <button class="h7-btn h7-btn-ghost">Secondary</button>
        <div style="padding:7px 10px;border:1px solid var(--h7-border);border-radius:var(--h7-radius-sm);font-size:12px;background:var(--h7-bg-glass);">Regular text <span style="color:var(--h7-text-muted);">and muted detail</span></div>
        <div style="width:40px;height:28px;border-radius:8px;background:var(--h7-accent);box-shadow:0 4px 12px color-mix(in srgb, var(--h7-accent) 35%, transparent);"></div>
      </div>`;
    ...
    function syncInputs() {
      COLOR_FIELDS.forEach(f => {
        ...
        colorInputs[f.key].textInput.value = workingTheme[f.key] || '';
        try { colorInputs[f.key].swatch.value = extractColor(workingTheme[f.key] || '#000'); } catch {}
      });
      OTHER_FIELDS.forEach(f => {
        if (otherInputs[f.key]) otherInputs[f.key].value = workingTheme[f.key] || '';
      });
    }
    ...
    function renderThemeList() {
      ...
      Object.entries(all).forEach(([key, t]) => {
        ...
        item.className = 'h7-context-item h7-theme-card' + (t.name === workingTheme.name ? ' is-selected' : '');
        item.style.gap = '8px';
