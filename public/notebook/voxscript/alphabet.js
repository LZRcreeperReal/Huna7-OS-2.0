...
Huna7.VoxScript.Alphabet = (() => {
  const TT = {
    ...
    IMPORT: 'IMPORT', NEW: 'NEW', SAY: 'SAY', WAIT: 'WAIT', REPEAT: 'REPEAT',
    // Operators
  ...
  const KEYWORDS = {
    let: TT.LET, const: TT.LET, fn: TT.FN, if: TT.IF, else: TT.ELSE,
    ...
    import: TT.IMPORT, new: TT.NEW, say: TT.SAY, wait: TT.WAIT, repeat: TT.REPEAT,
  ...
  const tokenize = (src) => {
    ...
    while (i < src.length) {
      ...
      if (ch === '/' && peek(1) === '*') {
        ...
        if (i >= src.length - 1) throw new SyntaxError(`Unterminated block comment at line ${line}`);
      ...
      if (ch === '"' || ch === "'") {
        ...
        if (i >= src.length) throw new SyntaxError(`Unterminated string at line ${line}`);
        advance();
      ...
      switch (ch) {
        ...
        case '-': if (src[i] === '=') { advance(); addToken(TT.MINUS_EQ, '-='); } else addToken(TT.MINUS, '-'); break;
        ...
        case '&': if (src[i] === '&') { advance(); addToken(TT.AND, '&&'); } else throw new SyntaxError(`Unexpected '&' at line ${line}, column ${col - 1}`); break;
        case '|': if (src[i] === '|') { advance(); addToken(TT.OR, '||'); } else throw new SyntaxError(`Unexpected '|' at line ${line}, column ${col - 1}`); break;
        default: throw new SyntaxError(`Unexpected '${ch}' at line ${line}, column ${col - 1}`);
      }
    }
