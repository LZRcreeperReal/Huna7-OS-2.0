...
Huna7.VoxScript.Translator = (() => {
  ...
  const OP = {
    ...
    MAKE_ARRAY: 'MAKE_ARRAY', MAKE_OBJECT: 'MAKE_OBJECT', SET_INDEX: 'SET_INDEX',
  ...
  class Compiler {
    ...
    compile_Assignment(node) {
      ...
      } else if (node.target.nodeType === 'IndexAccess') {
        this.compile(node.target.object);
        this.compile(node.target.index);
        this.compile(node.value);
    ...
    compile_FnDecl(node) {
      ...
      const bytecode = fnCompiler.finalize();
      this.emit(OP.MAKE_FUNC, { name: node.name, params: node.params, code: bytecode.instructions, constants: bytecode.constants });
      this.emit(OP.STORE, node.name);
    }
    ...
    compile_ArrowFn(node) {
      ...
      fnCompiler.emit(OP.RETURN);
      const bytecode = fnCompiler.finalize();
      this.emit(OP.MAKE_ARROW, { params: node.params, code: bytecode.instructions, constants: bytecode.constants });
    }
    ...
    compile_Background(node) {
      ...
      const bytecode = fnCompiler.finalize();
      this.emit(OP.BACKGROUND, bytecode);
    }
