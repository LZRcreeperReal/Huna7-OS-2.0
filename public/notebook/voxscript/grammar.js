...
Huna7.VoxScript.Grammar = (() => {
  ...
  class Parser {
    ...
    endStatement() { this.match(TT().SEMICOLON); this.skipNewlines(); }

    parse() {
      const body = [];
      this.skipNewlines();
      while (!this.at(TT().EOF)) { body.push(this.statement()); this.skipNewlines(); }
    ...
    block() {
      ...
      const body = [];
      this.skipNewlines();
      while (!this.at(tt.RBRACE) && !this.at(tt.EOF)) { body.push(this.statement()); this.skipNewlines(); }
