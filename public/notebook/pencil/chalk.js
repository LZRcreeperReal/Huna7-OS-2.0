...
Huna7.Chalk = (() => {
  ...
  const spawn = (appId, options = {}) => {
    ...
    // Enforce 50 process limit
    const running = Array.from(_processes.values()).filter(p => p.status === 'running').length;
    if (running >= MAX_PROCESSES) {
      Huna7.Bulletin?.warning('System', `Maximum ${MAX_PROCESSES} apps open. Close some first.`);
      return null;
    }
