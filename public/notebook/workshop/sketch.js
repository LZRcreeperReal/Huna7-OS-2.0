...
Huna7.Sketch = (() => {
  ...
  const tabBar = (tabs, onSelect, activeIndex = 0) => {
    ...
    tabs.forEach((tab, i) => {
      const el = document.createElement('div');
      el.style.cssText = `padding:8px 16px;font-size:13px;cursor:pointer;border-bottom:2px solid transparent;white-space:nowrap;transition:all var(--h7-anim-speed) ease;color:var(--h7-text-muted);`;
    ...
    tabEls.forEach((el, i) => {
      ...
      el.addEventListener('click', () => { setActive(); onSelect && onSelect(i, tabs[i]); });
    });
    return bar;
  };
